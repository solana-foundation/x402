...
import { acceptVoucher } from "../../src/batch-settlement/server/accept";
import { MemoryChannelStore, type ChannelState } from "../../src/batch-settlement/store";
...
function seedState(overrides: Partial<ChannelState> = {}): ChannelState {
  return {
    channelConfig,
    channelId,
...
describe("batch-settlement SVM", () => {
  ...
  describe("offchain voucher acceptance", () => {
    it("advances by the exact request price and returns an idempotent replay", async () => {
      const store = new MemoryChannelStore();
      await store.put(seedState());
      const voucher = await signedVoucher(1_000n);
      const args = {
        channelId,
        cumulativeAmount: 1_000n,
      ...
      const first = await acceptVoucher(store, args);
      const replay = await acceptVoucher(store, args);
      expect(first).toMatchObject({ charged: 1_000n, ok: true, replay: false });
      expect(replay).toMatchObject({ charged: 0n, ok: true, replay: true });
    });

    it("rejects wrong cumulative, deposit overflow, short expiry, and closing state", async () => {
      const wrongStore = new MemoryChannelStore();
      await wrongStore.put(seedState());
      const wrong = await signedVoucher(999n);
      expect(
        await acceptVoucher(wrongStore, {
          channelId,
          cumulativeAmount: 999n,
          expiresAt: 0,
          now: 1_000,
          perRequest: 1_000n,
          signatureBase58: wrong.signature,
        }),
      ).toEqual({ ok: false, reason: BatchError.CUMULATIVE_AMOUNT_MISMATCH });

      const overflowStore = new MemoryChannelStore();
      await overflowStore.put(seedState({ deposit: 500n }));
      const overflow = await signedVoucher(1_000n);
      expect(
        await acceptVoucher(overflowStore, {
          channelId,
          cumulativeAmount: 1_000n,
          expiresAt: 0,
          now: 1_000,
          perRequest: 1_000n,
          signatureBase58: overflow.signature,
        }),
      ).toEqual({ ok: false, reason: BatchError.CUMULATIVE_EXCEEDS_DEPOSIT });

      const expiryStore = new MemoryChannelStore();
      await expiryStore.put(seedState());
      const expiring = await signedVoucher(1_000n, 1_500);
      expect(
        await acceptVoucher(expiryStore, {
          channelId,
          cumulativeAmount: 1_000n,
          expiresAt: 1_500,
          ...
          now: 1_000,
          perRequest: 1_000n,
          signatureBase58: expiring.signature,
        }),
      ).toEqual({ ok: false, reason: BatchError.EXPIRY_WINDOW_TOO_SHORT });

      const closingStore = new MemoryChannelStore();
      await closingStore.put(seedState({ status: "closing" }));
      expect(
        await acceptVoucher(closingStore, {
          channelId,
          cumulativeAmount: 1_000n,
          expiresAt: 0,
          now: 1_000,
      ...
      ).toEqual({ ok: false, reason: BatchError.CHANNEL_CLOSING });
    });
  });
