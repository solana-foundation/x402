...
/** Build the permissionless `seal` transition after a forced-close grace period. */
export function buildSealInstruction(args: {
  channelId: string;
  programId?: Address | undefined;
}): ServerInstruction {
  return getSealInstruction(
    { channel: address(args.channelId) },
    { programAddress: args.programId ?? PAYMENT_CHANNELS_PROGRAM_ID },
  ) as ServerInstruction;
}
