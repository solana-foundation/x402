...
/**
 * SVM resource-server implementation for the `batch-settlement` payment scheme.
 *
 * Price parsing matches the `exact`/`upto` schemes (stablecoin → 6-decimal
 * atomic units); `amount` is the per-request price. `enhancePaymentRequirements`
 * folds the facilitator's `getExtra` (operator `facilitator`, `profiles`,
 * `channelProgram`, `gracePeriodSeconds`, …) into the requirement so the client
 * can open and reuse a payment channel.
 */
export class BatchSvmScheme implements SchemeNetworkServer {
  ...
  /**
   * Fold the facilitator's `getExtra` payload (operator address, channel program,
   * grace period, profiles, …) into the requirement so the client can open and
   * reuse a payment channel against this facilitator.
   *
   ...
   * @param supportedKind.extra - Facilitator extra (feePayer / channelProgram / profiles / …)
