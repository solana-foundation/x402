...
export class BatchSvmScheme implements SchemeNetworkServer {
  ...
  > {
    ...
    try {
      ...
      if (raw.type !== "deposit" && !state) throw new Error(BatchError.CHANNEL_NOT_FOUND);
  ...
  > {
    ...
    if (request.replay) {
      return {
        skipHandler: true,
        response: { body: { channelId: request.channelId, replay: true } },
      };
    }
  ...
  > {
    ...
    if (request?.replay) {
      ...
      this.requestContexts.delete(ctx.paymentPayload);
      if (!state) return this.abort(BatchError.CHANNEL_NOT_FOUND, "missing replay state");
  ...
  private async clearReservation(payload: DeepReadonly<PaymentPayload>): Promise<void> {
    ...
    if (!request?.pendingId) return;
    await this.store.update(request.channelId, current => {
      if (!current) throw new Error(BatchError.CHANNEL_NOT_FOUND);
  ...
  private provisionalState(
    ...
    if (raw.type !== "deposit") throw new Error(BatchError.CHANNEL_NOT_FOUND);
    const extra = requirements.extra!;
    return {
...
function acceptedResponse(
  ...
  return {
    amount: "",
    extra: {
      channelState: snapshot(state, committed ? BigInt(requirements.amount) : 0n),
...
function snapshot(state: ChannelState, pendingIncrement = 0n) {
  return {
    balance: state.deposit.toString(),
    channelId: state.channelId,
    chargedCumulativeAmount: (state.chargedCumulativeAmount + pendingIncrement).toString(),
