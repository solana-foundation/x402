...
export async function acceptVoucher(
  ...
  try {
    const state = await store.update(args.channelId, async current => {
      ...
      if (current.status === "distributed") throw new Rejection(BatchError.CHANNEL_CLOSED);
      ...
      if (args.expiresAt !== 0) {
        if (args.expiresAt <= args.now) throw new Rejection(BatchError.EXPIRY_WINDOW_TOO_SHORT);
        if (
          args.minExpiryWindowSeconds !== undefined &&
          args.minExpiryWindowSeconds > 0 &&
          args.expiresAt < args.now + args.minExpiryWindowSeconds
        ) {
          throw new Rejection(BatchError.EXPIRY_WINDOW_TOO_SHORT);
        }
      }

      // Idempotent replay: identical cumulative + signature is a no-op re-serve.
      if (args.cumulativeAmount === current.signedMaxClaimable) {
        if (!(await verifyVoucherSig(args, current.payerAuthorizer))) {
          throw new Rejection(BatchError.VOUCHER_SIGNATURE);
        }
        charged = 0n;
        replay = true;
        return current;
      ...
      if (args.cumulativeAmount <= current.signedMaxClaimable) {
        throw new Rejection(BatchError.CUMULATIVE_AMOUNT_MISMATCH);
      }
      if (args.cumulativeAmount > current.deposit) {
        throw new Rejection(BatchError.CUMULATIVE_EXCEEDS_DEPOSIT);
      }
      const delta = args.cumulativeAmount - current.chargedCumulativeAmount;
      ...
      return {
        ...current,
        chargedCumulativeAmount: args.cumulativeAmount,
        highestVoucherExpiresAt: args.expiresAt,
        highestVoucherSignature: args.signatureBase58,
        signedMaxClaimable: args.cumulativeAmount,
      };
    });
 ...
 * @param signer - Expected Ed25519 voucher signer
 * @returns Whether the signature is valid
 */
function verifyVoucherSig(args: AcceptVoucherArgs, signer: string): Promise<boolean> {
