...
The facilitator's `/supported` `extra` advertises `feePayer` and, optionally,
`transactionVersions`. The server returns those values together with its own `receiverAuthorizer` and
